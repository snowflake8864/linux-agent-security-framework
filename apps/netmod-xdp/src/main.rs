...
struct Args {
    ...
    #[arg(short, long, default_value = "../bpf/xdp_pkt_mod.bpf.o")]
    bpf_obj: String,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Attach XDP program
    let program: &mut Xdp = bpf_handle
        .program_mut("xdp_packet_filter")
        .unwrap()
        .try_into()?;
    program.load()?;
    program.attach(&args.interface, aya::programs::XdpFlags::default())?;

    println!("🔌 Attached XDP hook to interface: {}", args.interface);
    ...
    cleanup_program(&args.interface, true);
    ...
    let packet_mod_rules = vec![
        PacketModRule {
            match_rule: PacketMatch {
                ...
                direction: ebpf_common::NetDirection::Ingress,
                src_port: 0,
                dst_port: 443,
                dst_ip: [0, 0, 0, 0],
            },
